...
const DropdownLink = ({ workSanskritName, to, eventKey }: DropdownLinkProps) => {
...
const NavMenu = ({ useClass = '' }: NavMenuProps) => {
  const linkStyle = {
    marginTop: 'auto',
    marginBottom: 'auto',
    width: 'fit-content',
  ...
  const { allTextJson } = useStaticQuery<{
    allTextJson: {
      group: Array<{
        nodes: Array<{
          work: string;
  ...
  return (
    <Nav className={useClass}>
      {isMobile ? (
        ...
        >
          <Nav.Link as={Link} to={toUrl('/texts')} key="texts" style={{ ...linkStyle, ...dropdownStyle }}>
            Texts
          </Nav.Link>
          <Dropdown.Toggle
            as={Nav.Link}
          ...
          />
          <Dropdown.Menu style={{ ...linkStyle, paddingLeft: '0px' }} renderOnMount={true}>
